...
namespace TraceLab.Components.DevelopmentKit
{
    /// <summary>
    ...
    public class TermDocumentMatrix
    {
        ...
        /// </summary>
        /// <param name="artifactsCollections">Artifacts collections</param>
        ...
        /// <summary>
        /// Returns a copy of the corpus as a TLArtifactsCollection.
        /// Only works on corpora with whole integer terms.
        /// </summary>
        /// <returns>Corpus as TLArtifactsCollection</returns>
        public TLArtifactsCollection ToTLArtifactsCollection()
        {
            ...
            for (int i = 0; i < NumDocs; i++)
            {
                ...
                {
                    if (_matrix[i][j] > 0.0 && Math.Abs(_matrix[i][j] - Math.Round(_matrix[i][j])) < Settings.Default.DoubleTolerance)
                    {
                        int kmax = Convert.ToInt32(Math.Round(_matrix[i][j]));
                        for (int k = 0; k < kmax; k++)
                        {
                            text.Add(GetTermName(j));
                        }
                    }
                }
                artifacts.Add(new TLArtifact(GetDocumentName(i), String.Join(" ", text)));
            }
            return artifacts;
        }
