...
public class VectorSpaceModel {
	 ...
	 */
	private THashMap<Document, THashMap<String, Double>> tfIdfWeights;
	...
	public VectorSpaceModel(Corpus corpus) {
		this.corpus = corpus;
		tfIdfWeights = new THashMap<Document, THashMap<String, Double>>();
	...
	private void createTfIdfWeights() {
		...
		for (Document document : corpus.getDocuments()) {
			THashMap<String, Double> weights = new THashMap<String, Double>();
	...
	private double getMagnitude(Document document) {
		double magnitude = 0;
		THashMap<String, Double> weights = tfIdfWeights.get(document);
	...
	private double getDotProduct(Document d1, Document d2) {
		double product = 0;
		THashMap<String, Double> weights1 = tfIdfWeights.get(d1);
		THashMap<String, Double> weights2 = tfIdfWeights.get(d2);
		
		for (String term : weights1.keySet()) {
			if(!weights1.contains(term) || !weights2.contains(term)) product += 0;
			else product += weights1.get(term) * weights2.get(term);
		}
